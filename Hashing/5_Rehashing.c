...
int loadFactor(){return count/SIZE;}
...
int hashFun(int ele) {return (ele%SIZE);}

struct element ** updateHT(struct element **ht,struct element **htOld)
{
    int newSize = getNearestPrime(2*SIZE);
    int i;
    for(i=0;i<SIZE;i++)
    {
        ht[i]->flag = htOld[i]->flag;
        ht[i]->value = htOld[i]->value;
    }
    while(i<newSize)
    {
        ht[i]->flag = 0;
